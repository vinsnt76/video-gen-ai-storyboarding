...
// Proxies render requests to JSON2Video API securely
...
export default async function handler(req, res) {
  ...
  // Format payload according to JSON2Video v2 inline scenes format
  ...
  try {
    ...
    if (!response.ok || !data.success) {
      return res.status(response.status || 400).json({ error: data.message || 'Render request failed' });
    }
